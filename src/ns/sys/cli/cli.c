...
// cli helper function
void
cli_process_line(char *buf, uint16_t buf_len)
{
...
int
cli_parse_long(char *str, long *l)
{
    char *endptr;
...
int
cli_parse_unsigned_long(char *str, unsigned long *ul)
{
    char *endptr;
...
void
cli_output_bytes(const uint8_t *bytes, uint8_t len)
{
    for (int i = 0; i < len; i++) {
